...
namespace Senparc.Weixin.MP.TenPayLibV3
{
    ...
    {
        public static string GetNewBillNo(string mchId)
        {
...
PROCESSING	请求已受理，请稍后使用原单号查询发放结果	二十分钟后查询,按照查询结果成功失败进行处理
        ...
        public static EnterprisePayResult SendEnterprisePay(string appId, string mchId, string tenPayKey,
         string openId, string tenPayCertPath, string check_name,
         string iP, int redPackAmount, string remark,string partner_trade_no, out string nonceStr, out string paySign)
        {
            nonceStr = TenPayV3Util.GetNoncestr();
            //RequestHandler packageReqHandler = new RequestHandler(null);
            ...
            RequestHandler packageReqHandler = new RequestHandler();
            packageReqHandler.SetParameter("mch_appid", appId);//商户账号appid
            packageReqHandler.SetParameter("mchid", mchId);		  //微信支付分配的商户号
            packageReqHandler.SetParameter("nonce_str", nonceStr);              //随机字符串，不长于32位
            packageReqHandler.SetParameter("partner_trade_no", partner_trade_no); //商户订单号，需保持唯一性
            packageReqHandler.SetParameter("openid", openId);  //接受收红包的用户的openId
            packageReqHandler.SetParameter("check_name", check_name); //NO_CHECK：不校验真实姓名   FORCE_CHECK：强校验真实姓名
            if ("FORCE_CHECK" == check_name)
            {
                 packageReqHandler.SetParameter("re_user_name", "");// 收款用户真实姓名。如果check_name设置为FORCE_CHECK，则必填用户真实姓名
            }
            packageReqHandler.SetParameter("amount", redPackAmount.ToString());                //企业付款金额，单位为分
            packageReqHandler.SetParameter("desc", remark);   //企业付款操作说明信息。必填。
            ...
            EnterprisePayResult normalReturn = new EnterprisePayResult
            {
                err_code = "",
                err_code_des = ""
            };
